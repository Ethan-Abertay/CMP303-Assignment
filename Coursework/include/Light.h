...
public:
	...
	void setSpotlightExponent(float f) { spotlightExponent = f; };
	void setBEnabled(bool b) { bEnabled = b; };
	void setAttenConst(float f) { attenConst = f; };
	void setAttenLinear(float f) { attenLinear = f; };
	void setAttenQuadratic(float f) { attenQuad = f; };
	...
	XMMATRIX getViewMatrix(int index = 0);			///< Get light view matrix for shadow mapping, returns XMMATRIX
	...
	float getSpotlightExponent() { return spotlightExponent; };
	bool getBEnabled() { return bEnabled; };
	bool* getBEnabled_Ptr() { return &bEnabled; };
	float& getAttenConst() { return attenConst; };
	float& getAttenLinear() { return attenLinear; };
	float& getAttenQuadratic() { return attenQuad; };
...
protected:
	...
	float specularPower;
	XMVECTOR position;
	XMMATRIX viewMatrix[6];		// Point light can have 6 different view matrices
	...
	bool bEnabled = true;
	float attenConst, attenLinear, attenQuad;
};
