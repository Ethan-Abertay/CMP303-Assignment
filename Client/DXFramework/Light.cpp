...
void Light::generateViewMatrix()
{
	...
	viewMatrix[0] = XMMatrixLookAtLH(position, position + dir, up);

	// If this is a point light and we need to generate 5 more view matrices
	if (lightType == LightType::POINT)
	{
		viewMatrix[1] = XMMatrixLookAtLH(position, position - dir, up);
		viewMatrix[2] = XMMatrixLookAtLH(position, position + right, up);
		viewMatrix[3] = XMMatrixLookAtLH(position, position - right, up);
		viewMatrix[4] = XMMatrixLookAtLH(position, position + up, -dir);
		viewMatrix[5] = XMMatrixLookAtLH(position, position - up, dir);
	}
}
...
void Light::setDirection(XMFLOAT3 f)
{
	direction = XMFLOAT3(f.x, f.y, f.z);
}
...
void Light::setPosition(XMFLOAT3 f)
{
	position = XMVectorSet(f.x, f.y, f.z, 1.f);
}
...
XMMATRIX Light::getViewMatrix(int index)
{
